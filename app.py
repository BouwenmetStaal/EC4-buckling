...
class Parametrization(vkt.Parametrization):
    welcome = vkt.Text(
        "# EC4 Buckling calculation \n## Flexural buckling analysis according to EN-1994-1-1 art. 6.7.3\n Define your input here:"
    )
...
class Controller(vkt.Controller):
    ...
    @ImageView("Criteria checks", duration_guess=1)
    def display_plot(self, params, **kwargs):
        buis = BuisEigenschappen(
            t=params.webthickness, diameter=params.diameter, fyd=params.fy
        )
        # Assume criteria_plot() returns a matplotlib Figure
        fig, _ = buis.figure_criteria_info()
        text, _ =buis.criteria_info()
        print(text)
        fig.show()
        svg_data = StringIO()
    ...
    def createPlot_1(self, params, **kwargs):
        ...
        fig_2, _ = staalbeton.return_diagrams()

        fig_2.show()
        svg_data = StringIO()
        fig_2.savefig(svg_data, format="svg")
        return ImageResult(svg_data)
...
# viktor-cli publish --registered-name ec4-buckling --tag v0.0.7
